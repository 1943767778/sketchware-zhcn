...
public class MainActivity extends BasePermissionAppCompatActivity {
                        ...
                        if (wasSuccessful) {
                            ...
                            if (BackupFactory.zipContainsFile(path, "local_libs")) {
                                new MaterialAlertDialogBuilder(MainActivity.this)
                                        ...
                                        .setPositiveButton("复制", (dialog, which) -> manager.doRestore(path, true))
                                        .setNegativeButton("不复制", (dialog, which) -> manager.doRestore(path, false))
    ...
    private BottomSheetDialogView getBottomSheetDialogView() {
        ...
        bottomSheetDialog.setTitle("v7.0.0 主要更新");
        ...
        bottomSheetDialog.setPositiveButton("查看更新", (dialog, which) -> {
    ...
    private void allFilesAccessCheck() {
        if (Build.VERSION.SDK_INT > 29) {
            ...
            if (!optOutFile.exists() && !granted) {
                ...
                dialog.setTitle("Android 11 存储访问");
                dialog.setMessage("从 Android 11 开始，Sketchware Pro 需要新权限以避免 " + "taking ages to build projects. Don't worry, we can't do more to storage than " + "with current granted permissions.");
                dialog.setPositiveButton(Helper.getResString(R.string.common_word_settings), (v, which) -> {
                    FileUtil.requestAllFilesAccessPermission(this);
                    v.dismiss();
                });
                dialog.setNegativeButton("跳过", null);
                dialog.setNeutralButton("不再显示", (v, which) -> {
                    try {
