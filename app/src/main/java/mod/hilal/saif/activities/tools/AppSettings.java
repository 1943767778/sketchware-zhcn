...
public class AppSettings extends BaseAppCompatActivity {
                ...
                if (files.size() > 1 || isDirectory) {
                    new MaterialAlertDialogBuilder(AppSettings.this)
                            ...
                            .setSingleChoiceItems(new String[]{"Delete"}, -1, (actionDialog, which) -> {
                                new MaterialAlertDialogBuilder(AppSettings.this)
                                        .setTitle("删除 " + (isDirectory ? "folder" : "file") + "?")
                                        .setMessage("确定要删除此 " + (isDirectory ? "folder" : "file") + " permanently? This cannot be undone.")
                            ...
                            .setSingleChoiceItems(new String[]{"Edit", "Delete"}, -1, (actionDialog, which) -> {
                                switch (which) {
                                    ...
                                    case 1 -> new MaterialAlertDialogBuilder(AppSettings.this)
                                            .setTitle("删除文件？")
                                            .setMessage("确定要永久删除此文件吗？此操作无法撤销。")
    ...
    private void signApkFileDialog() {
        ...
        apkPathDialog.setTitle("使用 testkey 签名 APK");
        ...
        apkPathDialog.setPositiveButton("继续", (v, which) -> {
            if (!isAPKSelected[0]) {
            ...
            if (new File(output_apk_path).exists()) {
                ...
                confirmOverwrite.setTitle("文件已存在");
                confirmOverwrite.setMessage("名为 " + output_apk_file_name + " already exists at /sketchware/signed_apk/.  Overwrite it?");

                confirmOverwrite.setNegativeButton(Helper.getResString(R.string.common_word_cancel), null);
                confirmOverwrite.setPositiveButton("覆盖", (view, which1) -> {
                    v.dismiss();
            ...
            public void run() {
                ...
                runOnUiThread(() -> {
                    if (ApkSigner.LogCallback.errorCount.get() == 0) {
                        ...
                        tv_progress.setText("发生错误。详情请查看日志。");
                    }
                });
            }
        }.start();
